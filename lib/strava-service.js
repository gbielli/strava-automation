...
export async function processActivityWebhook(webhookEvent) {
  ...
  try {
    ...
    console.log(
      `Activité récupérée: "${activity.name}" (Type: ${activity.type})`
    );
    ...
    console.log(
      `Analyse pour déterminer si l'activité ${activityId} est un fractionné...`
    );
    ...
    console.log(
      `Résultat: L'activité ${activityId} est-elle un fractionné? ${isIntervalWorkout}`
    );

    if (!isIntervalWorkout) {
      console.log(`L'activité ${activityId} n'est pas un fractionné, ignorée`);
      return {
        success: true,
    ...
    console.log(`${laps.length} laps récupérés pour l'activité ${activityId}`);
    ...
    console.log(`Description générée pour l'activité ${activityId}`);

    // Mettre à jour la description de l'activité (garder le titre original)
    console.log(`Mise à jour de la description pour l'activité ${activityId}`);
    await updateActivityTitleAndDescription(
      accessToken,
      activityId,
      activity.name, // Conserver le titre original
      newDescription
    );
    console.log(
