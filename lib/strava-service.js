...
export async function saveOrUpdateUser(tokenData) {
  ...
  try {
    // Chercher si l'utilisateur existe déjà par son identifiant Strava
...
export async function updateActivityDescription(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        `Erreur lors de la mise à jour sur Strava: ${
          errorData.message || response.statusText
        }`
      );
    }
    ...
    console.error(
      `Erreur lors de la mise à jour de la description pour l'activité ${activityId}:`,
      error
    );
    throw error;
  }
