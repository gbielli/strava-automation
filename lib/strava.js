 ...
 */
export async function analyzeRecentActivities(count = 5, analyzeAll = false) {
  try {
    ...
    if (analyzeAll) {
      // Analyser toutes les activités récupérées (pour l'affichage initial)
      for (const activity of activities) {
        const result = await analyzeActivity(tokenData.access_token, activity);
        results.push(result);
      }
    } else {
      // Mode CRON: analyser uniquement la dernière activité si c'est un intervalle
      const latestActivity = activities[0]; // La première est la plus récente

      // Vérifier si c'est un fractionné avant de procéder à l'analyse complète
      if (shouldProcessActivity(latestActivity)) {
        console.log(
          `Analyse de la dernière activité: "${latestActivity.name}" (fractionné détecté)`
        );
        ...
        // Pour l'affichage, ajouter les autres activités sans les analyser
        for (let i = 1; i < activities.length; i++) {
          results.push({
            activity: activities[i],
            success: true,
            message: "Activité récupérée sans analyse",
            processed: false,
          });
        }
      } else {
        console.log(
          `La dernière activité "${latestActivity.name}" n'est pas un fractionné, aucune analyse nécessaire`
        );
        // Ajouter toutes les activités sans analyse
        for (const activity of activities) {
          results.push({
            activity,
            success: true,
